			...
			chrome.runtime.sendMessage({query:profUrl}, function(response){
				...
				if (div.getElementsByClassName('dosanddonts').length === 0){ //dosanddonts class only present if professor has no ratings
					var overallScores = Array.prototype.slice.call( //get Nodelist of scores, turn to array
						div.getElementsByClassName('grade'),0,3);
					var tags = Array.prototype.slice.call(
						div.getElementsByClassName('tag-box'), 0, 7);
					var allScores = overallScores.concat(tags); //array containing all relevant scores
					var categories = ['Overall Score', 'Take Again?','Difficulty', ''];

					for (i=0; i<categories.length; i++){ //create divs
						scoreDiv = document.createElement('div');
						scoreDiv.className = 'rmp-score-div'+i;
						...
						var profGrade = overallScores[0].innerText;

						if(between(profGrade, 0.0,2.5)){
						score.className = 'rmp-num-score-low';
					} if(between(profGrade, 2.6,4.0)){
						score.className = 'rmp-num-score-mid';
					} if(between(profGrade, 4.1,5.0)){
						score.className = 'rmp-num-score-high';
					}

//console.log(getElementByXpath('//html/div[2]/span[2]'));

						//score.className = 'rmp-num-score-high';
						//scoreLabel.getElementsByClassName('rmp-num-score-high').setAttribute[0]('id', 'icon awesome-icon');
